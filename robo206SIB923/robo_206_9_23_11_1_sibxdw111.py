...
def sibxdw111():
    try:
        ...
        for colunas in ws1.iter_rows(min_row=15):
            tamanho1 = coluna_2
            posicao1 = tamanho1 + 1
            coluna_ref4 = (ws1.cell(row = 1, column= posicao1).column_letter)
            # print(coluna_ref4)
            coluna1 = str(colunas[coluna_1].value).upper().strip().split()
            coluna2 = int(colunas[coluna_2].value)
            # print(coluna2)
            resultado = coluna2 * 100
            porcentagem = str(resultado) + '%'
            variacao.append(porcentagem)
            
        contador = 15 
        for item in variacao:
            ws1[f'{coluna_ref4}{contador}'] = item
            contador += 1
        ...
        for colunas in ws1.iter_rows(min_row=15):
            ...
            if not '0' in coluna1:
                ...
                ws1[f'{coluna_ref4}{contador}'] = '-' + '100%'
                contador += 1

        for colunas in ws1.iter_rows(min_row=15, max_row=15):
            tamanho = len(colunas)
            real = tamanho - 4
            ws1.delete_cols(real)
